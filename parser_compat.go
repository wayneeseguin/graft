...
func ParseOpcallCompat(phase OperatorPhase, src string) (*Opcall, error) {
	...
	if !strings.HasPrefix(strings.TrimSpace(src), "((") || !strings.HasSuffix(strings.TrimSpace(src), "))") {
		DEBUG("ParseOpcallCompat: '%s' does not look like an operator expression", src)
		return nil, nil
	}
	
	DEBUG("ParseOpcallCompat: checking '%s' in phase %v", src, phase)
	
	// Check if we should use the enhanced parser
	if UseEnhancedParser || shouldUseEnhancedParser(src) {
		DEBUG("ParseOpcallCompat: Using enhanced parser for '%s'", src)
		...
		if err != nil {
			...
			DEBUG("Enhanced parser error for '%s': %v, falling back to original", src, err)
			return ParseOpcall(phase, src)
		}
		if result != nil {
			DEBUG("Enhanced parser succeeded for '%s' - operator: %T", src, result.op)
			return result, nil
		}
		// If enhanced parser returned nil, fall back to original parser
		// This ensures backward compatibility during transition
		DEBUG("Enhanced parser returned nil for '%s', falling back to original", src)
	}
	
	// Use original parser
	DEBUG("ParseOpcallCompat: Using original parser for '%s'", src)
	return ParseOpcall(phase, src)
}
